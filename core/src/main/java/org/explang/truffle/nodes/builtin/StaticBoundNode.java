 ...
 */
public final class StaticBoundNode<T> extends ExpressionNode<T> {
  /** Builds a node statically resolving to a built-in function */
  public static ExpressionNode<ExplFunction> builtIn(BuiltInNode<?> builtin) {
    return function(BuiltInNode.createBuiltin(builtin));
  }

  /** Builds a node statically resolving to a function */
  public static ExpressionNode<ExplFunction> function(ExplFunction f) {
    return new ExpressionNode<ExplFunction>() {
      @Override
