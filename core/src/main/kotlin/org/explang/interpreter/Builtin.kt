...
abstract class BuiltinFunction(
    ...
    val body: (List<EvalResult>, EvalContext) -> Any) : Callable {
  override fun closure(): MutableMap<String, EvalResult> = mutableMapOf()

  override fun invoke(frame: Frame, ctx: EvalContext): EvalResult {
    return EvalResult(body(frame.args, ctx))
  }
}
...
val BUILTINS = listOf(
    ...
    make("filter", function(array(DOUBLE), array(DOUBLE), function(BOOL, DOUBLE))) { args, ctx ->
      ...
      r.filter { v -> ctx.call(f, listOf(EvalResult(v))).value as Boolean }
    },
    make("map", function(array(DOUBLE), array(DOUBLE), function(DOUBLE, DOUBLE))) { args, ctx ->
      val r = args[0].value as ArrayValue<Double>
      val f = args[1].value as Callable
      mapToDouble(r) { v -> ctx.call(f, listOf(EvalResult(v))).value as Double }
    },
    make("fold", function(DOUBLE, array(DOUBLE), DOUBLE, function(DOUBLE, DOUBLE, DOUBLE))) { args, ctx ->
      ...
      fold(r, init) { acc, v -> ctx.call(f, listOf(EvalResult(acc), EvalResult(v))).value as Double }
    },
    make("reduce", function(DOUBLE, array(DOUBLE), function(DOUBLE, DOUBLE, DOUBLE))) { args, ctx ->
      val r = args[0].value as ArrayValue<Double>
      val f = args[1].value as Callable
      reduce(r) { acc, v -> ctx.call(f, listOf(EvalResult(acc), EvalResult(v))).value as Double }
    },
    ...
    make("mapr", function(array(LONG), range(LONG), function(LONG, LONG))) { args, ctx ->
      ...
      val f = args[1].value as Callable
      mapToLong(r) { v -> ctx.call(f, listOf(EvalResult(v))).value as Long }
    },
    make("foldr", function(LONG, array(LONG), LONG, function(LONG, LONG, LONG))) { args, ctx ->
      val r = args[0].value as RangeValue<Long>
      val init = args[1].value as Long
      val f = args[2].value as Callable
      fold(r, init) { acc, v -> ctx.call(f, listOf(EvalResult(acc), EvalResult(v))).value as Long }
    },
    make("reducer", function(LONG, array(LONG), function(LONG, LONG, LONG))) { args, ctx ->
      val r = args[0].value as RangeValue<Long>
      val f = args[1].value as Callable
      reduce(r) { acc, v -> ctx.call(f, listOf(EvalResult(acc), EvalResult(v))).value as Long }
    }
)

fun make(name: String, type: FuncType, body: (List<EvalResult>, EvalContext) -> Any): BuiltinFunction {
